...
(function () {
  ...
  function renderExportCenter() {
    ...
    const groups = exportGroups.map(([key, label]) => {
      const buttons = Object.entries(exportTypes).filter(([, v]) => v.section === key).map(([id, item]) => `<button class="btn ${state.exportType === id ? 'active primary' : ''}" onclick="WaterFuel.setExportType('${id}')">${esc(item.label)}</button>`).join('');
