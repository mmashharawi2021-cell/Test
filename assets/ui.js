window.AppUI = (() => {
  ...
  function themeDots() {
    ...
    return `<div class="theme-dock" title="تغيير لون الموقع"><span class="theme-dock-label">ألوان</span>${dots.map(([key, label]) => `<button type="button" class="theme-dot theme-dot-${key}" data-theme-dot="${key}" aria-label="${label}" onclick="ThemeManager.saveUserTheme('${key}')"><i></i></button>`).join('')}</div>`;
  }
  ...
  function layout(state) {
    ...
    return `<main class="app-shell">${themeDots()}<header class="hero"><div><p class="eyebrow">لوحة التشغيل</p><h1>نظام تقارير تشغيل وضخ المياه</h1><p>منصة يومية رسمية للتشغيل، الوقود، الإنتاج، الفحوصات، الجهات المستفيدة، والأرشفة.</p></div><div class="hero-actions"><button class="btn primary big action-float" onclick="App.openNew()">➕ إضافة تقرير جديد</button><button class="btn" onclick="App.openSummary()">📈 تقارير تجميعية</button><button class="btn" onclick="App.exportAllExcel()">📊 Excel شامل</button><button class="btn ghost" onclick="App.logout()">🚪 خروج</button></div></header><section class="stats dashboard-totals"><article><span>عدد التقارير</span><strong>${reports.length}</strong></article><article><span>إجمالي ساعات التشغيل</span><strong>${s.runHours.toFixed(1)}</strong></article><article><span>إجمالي الوقود المستهلك</span><strong>${s.fuelConsumed}</strong></article><article><span>إجمالي المياه المعبأة</span><strong>${s.filledWater}</strong></article><article><span>إجمالي الإنتاج</span><strong>${s.waterProduction}</strong></article><article><span>إجمالي العادم</span><strong>${s.rejectWater}</strong></article><article><span>إجمالي السيارات</span><strong>${s.cars}</strong></article><article><span>نسبة الفاقد</span><strong>${s.lossPercentage}%</strong></article></section><section class="cards-section"><div class="section-head"><div><p class="eyebrow">الأرشيف</p><h2>كروت التقارير</h2></div></div><div class="cards reports-grid">${reports.map(r => card(r, active?.id)).join('') || '<div class="empty-mini">لا توجد تقارير محفوظة.</div>'}</div></section>${details(active)}<div class="bottom-space"></div>${modal()}</main>`;
  }
